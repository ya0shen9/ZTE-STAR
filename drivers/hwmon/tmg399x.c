...
static bool cal_poffset_finished = false;
...
/*
	...
	s3c_gpio_cfgpin(GPIO_tmg399x_IRQ, S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(GPIO_tmg399x_IRQ, S3C_GPIO_PULL_UP);


	return 0;

exit_free_gpio:
	gpio_free(GPIO_tmg399x_IRQ);
	printk(KERN_INFO "board_tmg399x_init CALLED\n");
*/
	printk(KERN_INFO "board_tmg399x_init CALLED\n");
...
/*
	int ret;
	printk(KERN_INFO "board_tmg399x_power CALLED\n");
	if (state == GPOWER_ON ) {
		ret = gpio_request(GPIO_tmg399x_IRQ,"tmg399x_int");
		if (ret < 0) {
			dev_dbg(dev, "%s: gpio request failed\n", __func__);
		}
		s3c_gpio_cfgpin(GPIO_tmg399x_IRQ, S3C_GPIO_SFN(GPIO_FUNC_EINT7));
		s3c_gpio_setpull(GPIO_tmg399x_IRQ, S3C_GPIO_PULL_UP);
	}
...
{
	printk(KERN_INFO "board_tmg399x_teardow CALLED\n");
/*
	dev_dbg(dev, "%s\n", __func__);
	gpio_free(GPIO_tmg399x_IRQ);
	if (tmg399x_vreg) {
		regulator_put(tmg399x_vreg);
		tmg399x_vreg = NULL;
	}
	*/
}
...
struct tmg399x_i2c_platform_data tmg399x_data = {
	...
	.parameters = {
		...
		.persist = PRX_PERSIST(0) | ALS_PERSIST(0),
		.als_prox_cfg1 = 0x60,
		.prox_pulse = PPLEN_16US | PRX_PULSE_CNT(4),
		.prox_gain = GPGAIN_4,
		.ldrive = GPDRIVE_100MA,
		.als_prox_cfg2 = LEDBOOST_150 | 0x01,
		.prox_offset_ne = 0,
		...
		.als_prox_cfg3 = 0,
            ...
            .ges_offset_n = 0,
            .ges_offset_s = 0,
            .ges_pulse = GPLEN_16US | GES_PULSE_CNT(6),
            .ges_offset_w = 0,
            .ges_offset_e = 0,
            .ges_dimension = GBOTH_PAIR,
	},
...
void tmg3993_gesture_zoom(int p_data)
{
	static u8 current_level = 0,old_level = 0;
	static int zoom_level = 0;
	u8 i = 0;
	
	current_level = p_data/80;
		
	if((current_level != old_level) && (current_level != 0))
	{
		zoom_level = current_level - old_level;
		old_level = current_level;
		
//		printk(KERN_INFO"gesture_zoom--->p_data=%d,currennt_level=%d,zoom_level=%d\n",p_data,current_level,zoom_level);
		
		if(zoom_level > 0)
		{
			for(i = 0; i < zoom_level; i++)
			gesture_report_status_zoom(0,10);
		}
		else
		{
			for(i = 0; i < (0-zoom_level); i++)
			gesture_report_status_zoom(1,10);
		}
	}
}
...
//	copy_from_user(value,buf,13);

//	for (i = 0; i <= ARRAY_SIZE(value); i++) 
//	{
//		printk(KERN_INFO "value[%d] = %x\n",i,value[i]);
//	}
	...
	tmg399x_i2c_ram_blk_write(chip, 0x00, chip->bc_symbol_table,
			sizeof(chip->bc_symbol_table));
...
static int tmg399x_set_op_mode(struct tmg399x_chip *chip, u8 mode, u8 on)
{
	...
	case ALS_PROX:
		...
		if (chip->in_hilight) {
			chip->shadow[TMG399X_PRX_PULSE] = PPLEN_4US | PRX_PULSE_CNT(12);
		} else {
			chip->shadow[TMG399X_PRX_PULSE] = PPLEN_16US | PRX_PULSE_CNT(3);
		}
	...
	case ALS_PROX_GES:
		...
		if (chip->in_hilight) {
			chip->shadow[TMG399X_PRX_PULSE] = PPLEN_4US | PRX_PULSE_CNT(24);
			chip->shadow[TMG399X_GES_PULSE] = GPLEN_4US | GES_PULSE_CNT(48);
		} else {
			chip->shadow[TMG399X_PRX_PULSE] = PPLEN_16US | PRX_PULSE_CNT(6);
			chip->shadow[TMG399X_GES_PULSE] = GPLEN_16US | GES_PULSE_CNT(12);
		}
	...
	ret |= tmg399x_i2c_write(chip, TMG399X_ALS_TIME, chip->shadow[TMG399X_ALS_TIME]);
	ret |= tmg399x_i2c_write(chip, TMG399X_PRX_PULSE, chip->shadow[TMG399X_PRX_PULSE]);
...
static int tmg399x_ges_enable(struct tmg399x_chip *chip, int on)
{
	...
	ret = tmg399x_i2c_write(chip, TMG399X_CONTROL, 0);
	if (ret < 0)
		return ret;
...
static int tmg399x_prox_enable(struct tmg399x_chip *chip, int on)
{
	...
	if (on) {
		...
		// gongyahui report 99 when disable, update input status 
		//input_report_abs(chip->p_idev, ABS_DISTANCE, 99);
		//input_sync(chip->p_idev);
	}
...
static int tmg399x_als_enable(struct tmg399x_chip *chip, int on)
{
	...
	if (on) {
		...
		chip->rgbc_timer.expires = jiffies + HZ/5;
...
static ssize_t tmg399x_ges_touch_store(struct device *dev,struct device_attribute *attr,
										const char *buf, size_t size)
{
	u8 ges_state;
	int rc;
//	int i = 0;
	...
	rc = kstrtou8(buf, 10, &ges_state);
	if (rc)
		return -EINVAL;
	
	dev_info(dev, "%s: ges_touch_state = %d\n", __func__,ges_state);
	...
	switch(ges_state&GES_MOTION_MASK)
	{
		...
		//test event	
		case 20:
			gesture_report_status_zoom(0,10);//zoom in
			break;
		...
		case 31:
			gesture_touch_pressed(1, 5);
			break;

		case 32:
			gesture_touch_pressed(2, 5);
			break;	

		case 33:
			gesture_touch_pressed(3, 5);
			break;

		case 34:
			gesture_touch_pressed(4, 5);
			break;
...
static struct device_attribute prox_attrs[] = {
	__ATTR(prx_power_state, 0660, tmg399x_prox_enable_show,
			tmg399x_prox_enable_store),
	__ATTR(ges_power_state, 0660, tmg399x_ges_enable_show,
			tmg399x_ges_enable_store),
	__ATTR(prx_persist, 0660, tmg399x_prox_persist_show,
			tmg399x_prox_persist_store),
	__ATTR(prx_pulse_length, 0660, tmg399x_prox_pulse_len_show,
			tmg399x_prox_pulse_len_store),
	__ATTR(prx_pulse_count, 0660, tmg399x_prox_pulse_cnt_show,
			tmg399x_prox_pulse_cnt_store),
	__ATTR(prx_gain, 0660, tmg399x_prox_gain_show,
			tmg399x_prox_gain_store),
	__ATTR(led_drive, 0660, tmg399x_led_drive_show,
			tmg399x_led_drive_store),
	__ATTR(led_boost, 0660, tmg399x_led_boost_show,
			tmg399x_led_boost_store),
	__ATTR(prx_sat_irq_en, 0660, tmg399x_sat_irq_en_show,
			tmg399x_sat_irq_en_store),
	__ATTR(prx_offset_ne, 0660, tmg399x_prox_offset_ne_show,
			tmg399x_prox_offset_ne_store),
	__ATTR(prx_offset_sw, 0660, tmg399x_prox_offset_sw_show,
			tmg399x_prox_offset_sw_store),
	__ATTR(prx_mask, 0660, tmg399x_prox_mask_show,
			tmg399x_prox_mask_store),
	__ATTR(prx_raw, 0660, tmg399x_device_prx_raw, NULL),
	__ATTR(prx_detect, 0660, tmg399x_device_prx_detected, NULL),
	__ATTR(ges_offset, 0660, tmg399x_ges_offset_show, NULL),

	__ATTR(ges_motion_state, 0660, tmg399x_ges_motion_show, NULL),
	__ATTR(ges_touch, 0660, NULL,tmg399x_ges_touch_store),
	__ATTR(ges_work_mode, 0660, tmg399x_ges_work_mode_show,tmg399x_ges_work_mode_store),
	__ATTR(ges_touch_style_state, 0660, NULL,tmg399x_ges_touch_style_store),
	__ATTR(ges_touch_dir8_state, 0660, NULL,tmg399x_ges_touch_dir8_store),
		
	__ATTR(beam_xmit, 0660, NULL, tmg399x_xmit_beam),
	__ATTR(beam_table, 0660, NULL, tmg399x_beam_symbol_store),
	__ATTR(beam_hop, 0660, tmg399x_beam_hop_show, tmg399x_beam_hop_store),
	__ATTR(beam_power_state, 0660, tmg399x_beam_enable_show,tmg399x_beam_enable_store),
};

static struct device_attribute als_attrs[] = {
	__ATTR(als_power_state, 0660, tmg399x_als_enable_show,
			tmg399x_als_enable_store),
	__ATTR(wait_time_en, 0660, tmg399x_wait_enable_show,
			tmg399x_wait_enable_store),
	__ATTR(als_Itime, 0660, tmg399x_als_itime_show,
		tmg399x_als_itime_store),
	__ATTR(wait_time, 0660, tmg399x_wait_time_show,
		tmg399x_wait_time_store),
	__ATTR(als_persist, 0660, tmg399x_als_persist_show,
			tmg399x_als_persist_store),
	__ATTR(als_gain, 0660, tmg399x_als_gain_show,
			tmg399x_als_gain_store),
	__ATTR(lux_table, 0660, tmg399x_lux_table_show,
			tmg399x_lux_table_store),
	__ATTR(als_auto_gain, 0660, tmg399x_auto_gain_enable_show,
			tmg399x_auto_gain_enable_store),
	__ATTR(als_lux, 0660, tmg399x_als_lux_show, NULL),
	__ATTR(als_red, 0660, tmg399x_als_red_show, NULL),
	__ATTR(als_green, 0660, tmg399x_als_green_show, NULL),
	__ATTR(als_blue, 0660, tmg399x_als_blue_show, NULL),
	__ATTR(als_clear, 0660, tmg399x_als_clear_show, NULL),
	__ATTR(als_cct, 0660, tmg399x_als_cct_show, NULL),
};
...
void tmg399x_report_prox(struct tmg399x_chip *chip, u8 detected)
{
	if (chip->p_idev) {
		...
		input_report_abs(chip->p_idev, ABS_DISTANCE,
				chip->prx_inf.detected ? 0 : 6);
		input_sync(chip->p_idev);
	}

	if (!(last_mode & GES_FUNC)) {
...
void tmg399x_report_ges(struct tmg399x_chip *chip, u8 ges_data)
{
	if (chip->g_idev) {
		...
		input_report_abs(chip->g_idev, ABS_DISTANCE,ges_data);
...
void tmg399x_report_als(struct tmg399x_chip *chip)
{
	...
	chip->als_inf.clear_raw = le16_to_cpup((const __le16 *)&chip->shadow[TMG399X_CLR_CHANLO]);
	chip->als_inf.red_raw = le16_to_cpup((const __le16 *)&chip->shadow[TMG399X_RED_CHANLO]);
	chip->als_inf.green_raw = le16_to_cpup((const __le16 *)&chip->shadow[TMG399X_GRN_CHANLO]);
	chip->als_inf.blue_raw = le16_to_cpup((const __le16 *)&chip->shadow[TMG399X_BLU_CHANLO]);
	...
	mod_timer(&chip->rgbc_timer, jiffies + HZ/5);

	printk(KERN_INFO "lux=%d  cct=%d\n", chip->als_inf.lux, chip->als_inf.cct);
	lux = chip->als_inf.lux;
	input_report_abs(chip->a_idev, ABS_X, lux);
...
void tmg399x_start_calibration(struct tmg399x_chip *chip)
{
	...
	tmg399x_i2c_modify(chip, TMG399X_GES_CFG_4, 0x01, 0x01);
	chip->shadow[TMG399X_PERSISTENCE] = 0x00;
	chip->params.persist = chip->shadow[TMG399X_PERSISTENCE];
		...
		if (chip->prx_enabled)
			...
			if (pstatechanged) {
				...
				if (docalibration && caloffsetstate != CHECK_NSWE_ZERO) {
					...
					if (cal_poffset_finished) {
						cal_poffset_finished = false;
						if (chip->shadow[TMG399X_PRX_CHAN] < (PROX_THRESH_HIGH * chip->prox_th_factor)) {
							input_report_abs(chip->p_idev, ABS_DISTANCE, 6);
							input_sync(chip->p_idev);
						}

						if (!(last_mode & GES_FUNC)) {
							chip->shadow[TMG399X_PERSISTENCE] = 0x10;
							chip->params.persist = chip->shadow[TMG399X_PERSISTENCE];
							ret = tmg399x_i2c_write(chip, TMG399X_PERSISTENCE,
							chip->shadow[TMG399X_PERSISTENCE]);
						}
...
// gongyahui added close poweron by gesturs, 
	...
	const struct i2c_device_id *idp)
{
	...
	set_bit(ABS_DISTANCE, chip->g_idev->absbit);
...
#define 	PRESS_MAX       	255
	...
	input_set_abs_params(chip->p_idev,ABS_MT_POSITION_X, 0, SCREEN_MAX_X, 0, 0);
	input_set_abs_params(chip->p_idev,ABS_MT_POSITION_Y, 0, SCREEN_MAX_Y, 0, 0);
	input_set_abs_params(chip->p_idev,ABS_MT_TOUCH_MAJOR, 0, PRESS_MAX, 0, 0);
	input_set_abs_params(chip->p_idev,ABS_MT_WIDTH_MAJOR, 0, 200, 0, 0);
	input_set_abs_params(chip->p_idev, ABS_MT_TRACKING_ID, 0, 2, 0, 0);
...
{
// gongyahui added close poweron by gesturs, 
// for sys can not wakeup when suspend
// gesture whill reinit when wakeup. if want to open the function, will update this driver.
...
static struct i2c_device_id tmg399x_idtable[] = {
	{ "tmg399x,ams-gesture", 0 },
	{}
};


#ifdef CONFIG_OF
static struct of_device_id tmg399x_match_idtable[] = {
	{ .compatible = "tmg399x,ams-gesture"},//wangzhanmeng modify 20130511
	{ },
};
#endif
...
static struct i2c_driver tmg399x_driver = {
	.driver = {
		.name = "tmg399x,ams-gesture",
		.pm = &tmg399x_pm_ops,
		.owner = THIS_MODULE,

		#ifdef CONFIG_OF
		.of_match_table = tmg399x_match_idtable,
		#endif
