...
struct sGesture_DS enter_dir_event;
struct sGesture_DS exit_dir_event;
...
extern void tmg399x_ges_touch_state(struct tmg399x_chip *chip, u8 ges_state);
...
{
	ges_exit_threshold = get_nswe_max(nswe_offset) + 6;
...
{
	stdev_filter100_10 = stdev_default100 * 10;
...
static void set_thresholds(int avg10, int delta10)
{
	...
	prox_exit_threshold = prox_entry_threshold;
	//printk(KERN_INFO "TH: entry:%d\texit:%d\n", prox_entry_threshold, prox_exit_threshold);
}
...
static void ges_output_dir4_event(void)
{
	if(ges_event.dir4.function == Cross&& ges_event.dir4.exit_dir== 0)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 0;
		printk(KERN_INFO "->right\n");
	}
	else if(ges_event.dir4.function == Cross && ges_event.dir4.exit_dir  == 2)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 2;
	...
	else if(ges_event.dir4.function == Cross && ges_event.dir4.exit_dir == 4)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 4;
	...
	else if(ges_event.dir4.function == Cross&& ges_event.dir4.exit_dir == 6)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 6;
	...
	else if(ges_event.dir4.function == Tap && ges_event.dir4.exit_dir == 0)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 11;
		printk(KERN_INFO "->right tap\n");
	}
	else if(ges_event.dir4.function == Tap && ges_event.dir4.exit_dir == 2)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 12;
		printk(KERN_INFO "->down tap\n");
	}
	else if(ges_event.dir4.function == Tap&& ges_event.dir4.exit_dir == 4)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 13;
		printk(KERN_INFO "->left tap\n");
	}
	else if(ges_event.dir4.function == Tap && ges_event.dir4.exit_dir == 6)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 14;
		printk(KERN_INFO "->up tap\n");
	}
	else
	{
		ges_motion = 0x80;
...
static void ges_output_dir8_event(void)
{
	if(ges_event.dir8.function == Cross&& ges_event.dir8.exit_dir== 0)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 0;
	...
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir  == 1)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 1;
		printk(KERN_INFO "->right up\n");
	}
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir  == 2)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 2;
	...
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir  == 3)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 3;
		printk(KERN_INFO "->left up\n");
	}
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir == 4)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 4;
	...
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir  == 5)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 5;
		printk(KERN_INFO "->left down\n");
	}
	else if(ges_event.dir8.function == Cross&& ges_event.dir8.exit_dir == 6)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 6;
	...
	else if(ges_event.dir8.function == Cross && ges_event.dir8.exit_dir  == 7)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 7;
		printk(KERN_INFO "->right down\n");
	}
	else if(ges_event.dir8.function == Tap && ges_event.dir8.exit_dir == 0)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 11;
	...
	else if(ges_event.dir8.function == Tap && ges_event.dir8.exit_dir == 2)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 12;
	...
	else if(ges_event.dir8.function == Tap&& ges_event.dir8.exit_dir == 4)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 13;
	...
	else if(ges_event.dir8.function == Tap && ges_event.dir8.exit_dir == 6)
	{
		ges_motion &= ~0x80;
		ges_motion &=~GES_MOTION_MASK;
		ges_motion |= 14;
...
static void ges_output_event(struct tmg399x_chip *chip,struct sGesture_DS enter_dir,struct sGesture_DS exit_dir)
{
	printk(KERN_INFO "--->ges_output_event.\n");
	
	ges_motion &=~GES_SPEED_MASK;
	if(ges_vector.duration_ms >= 80)	ges_motion |=GES_SPEED_SLOW;
	else ges_motion |=GES_SPEED_MID;

	if(ges_dir8_enable)ges_output_dir8_event();
	else ges_output_dir4_event();
		

	if(!(ges_motion & 0x80))//gesture is ok?
	{
		tmg399x_report_ges(chip,ges_motion);
		if(ges_touch_style) //true for touch event in kernel
		tmg399x_ges_work_mode(chip,work_mode,ges_motion);
	}
...
static void create_ges_event(struct tmg399x_chip *chip)
{
	...
	else
	{
		...
		get_ges_dfs4(&ges_event.dir4, enter_dir, exit_dir);
	        ...
	        if (ges_event.dir4.function == Tap)
	                printk(KERN_INFO "%s %d --> %s %d, Tap, duration:%ldms\n",
				direction[ges_event.dir4.enter_dir], ges_vector.enter_angle,
				direction[ges_event.dir4.exit_dir], ges_vector.exit_angle,
				ges_vector.duration_ms);
	        else {
	                printk(KERN_INFO "%s %d --> %s %d, Cross, duration:%ldms\n",
				direction[ges_event.dir4.enter_dir], ges_vector.enter_angle,
	...
	ges_output_event(chip,enter_dir,exit_dir);
...
static void check_long_ges(struct tmg399x_chip *chip)
{
	...
	if(work_mode == TMG_MODE_GAME || work_mode ==TMG_MODE_BOOK)
	{
		if (mrawdataarray.long_count == 0) 
		{
			...
			if(work_mode ==TMG_MODE_BOOK)
			{
			        if (ges_vector.enter_angle < 75 || ges_vector.enter_angle > 285) 
				{
					printk(KERN_INFO "North--->Enter ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 15;
					enter_dir_event.dir = North;
					
			        }
				else //if (ges_vector.enter_angle > (110) || ges_vector.enter_angle < 250) 
			        {
					printk(KERN_INFO "south--->Enter  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 17;
					enter_dir_event.dir = South;	
			        } 
			...
			else
			{
			        if (ges_vector.enter_angle < inc || ges_vector.enter_angle > (360 - inc)) 
				{
			        ...
			        {
					printk(KERN_INFO "East--->Enter  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 16;
					enter_dir_event.dir = East;
				...
				{
					printk(KERN_INFO "West--->Enter  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 18;
					enter_dir_event.dir = West;
			        }
			}
	...
	{
		if (mrawdataarray.long_count == 20) 
		{
			/* fisrt long count, it's pushed event */
			mrawdataarray.long_count++;
		...
		if (mrawdataarray.long_count > 400) {
		...
		if(mrawdataarray.long_count == 40)
		{
			ges_motion &= ~0x80;
			ges_motion &=~GES_MOTION_MASK;
			ges_motion |= 9;
	//		printk(KERN_INFO "button held 2 times\n");
			if(!(ges_motion & 0x80))//
			{
				if(ges_touch_style) //true for touch event in kernel
				tmg399x_ges_work_mode(chip,work_mode,ges_motion);
...
static void add_ges_rawdata(struct tmg399x_chip *chip, struct sRawData *prawdata)
{
	...
	if (chip->prx_enabled) {
		if (hidden_data) {
        	       	if (!prox_state && prawdata->gproxmax > (PROX_THRESH_HIGH * chip->prox_th_factor)) {
        	       	        /* report prox near event here */
        	       	        prox_state = true;
				tmg399x_report_prox(chip, prox_state);
        	       	} else if (prox_state && prawdata->gproxmax < (PROX_THRESH_LOW * chip->prox_th_factor)) {
	...
	case TENDED:
		...
		if(work_mode ==TMG_MODE_BOOK)
		{
			if (mrawdataarray.long_count > 0) 
			{
				...
				printk(KERN_INFO "released, duration:%ldms\n", duration_ms);

				process_ges_rawdataarray();
				create_ges_vector(0);

				if (ges_vector.exit_angle < 75 || ges_vector.exit_angle > 285) 
				{
					printk(KERN_INFO "North--->Exit ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 19;
					exit_dir_event.dir = North;
					
			        }
				 else //if (ges_vector.exit_angle > 110 || ges_vector.exit_angle < 250) 
			        {
					printk(KERN_INFO "East--->Exit  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 21;
					exit_dir_event.dir = South;
					
			        }

#if 0			
			        if (ges_vector.exit_angle < inc || ges_vector.exit_angle > (360 - inc)) 
				{
					printk(KERN_INFO "North--->Exit ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
			        ...
			        } else if (ges_vector.exit_angle < (inc + 90)) 
			        {
					printk(KERN_INFO "East--->Exit  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 20;
					exit_dir_event.dir = East;
					
			        } else if (ges_vector.exit_angle < (inc + 180))
			       {
					printk(KERN_INFO "South--->Exit  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 21;
					exit_dir_event.dir = South;
				...
				{
					printk(KERN_INFO "West--->Exit  ");
					ges_motion &= ~0x80;
					ges_motion &=~GES_MOTION_MASK;
					ges_motion |= 22;
					exit_dir_event.dir = West;
			        }
#endif		
				if(!(ges_motion & 0x80))//
				{
...
static void process_prox_data(struct tmg399x_chip *chip, u8 prox)
{
	...
	if (prox == 0) {
		...
		prox_zero_count = 0;
		if (chip->prx_enabled) {
			if (!prox_state && prox_data > (PROX_THRESH_HIGH * chip->prox_th_factor)) {
				/* report prox near event here */
				prox_state = true;
				tmg399x_report_prox(chip, prox_state);
			} else if (prox_state && prox_data < (PROX_THRESH_LOW * chip->prox_th_factor)) {
				/* report prox far event here */
				prox_state = false;
				tmg399x_report_prox(chip, prox_state);
			}
