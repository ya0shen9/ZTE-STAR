	...
	case SPI_IOC_WR_MAX_SPEED_HZ:
       ...
       case ICE40_IOCTL_CMD_POWER_CTRL:
            if(1 == state)
            {
                ...
                g_irda_open_flag = 1;
                #endif
                 printk("ice40_power_on\n");
                 retval = ice40_power_on();
            }
            else if(0 == state) 
            {
                #ifdef CONFIG_IR_LATTICE_LED
...
static int __devinit spidev_probe(struct spi_device *spi)
{
  ...
  ir_clk  = clk_get(&spi->dev, "ref_clk");
  if (ir_clk == NULL)
  {
    printk("lattice BBCLK2 clk_get fail \n");
  }
  else
  {
    irda_clk = ir_clk;
    #if 0
    r = clk_prepare_enable(ir_clk);
    if (r)
    {
      printk("lattic clk_prepare_enable fail \n");
      clk_disable_unprepare(ir_clk);
    }
    printk("lattice BBCLK2 clk_prepare_enable success! \n");
    #endif
  }
...
#ifdef CONFIG_IR_LATTICE_LED
int g_spidev_suspend = 0;

static int spidev_suspend(struct device *dev)
{
    g_spidev_suspend = 1;
    printk(KERN_ERR"spidev_suspend\n");
    return 0;
}
...
{
    g_spidev_suspend = 0;
    printk(KERN_ERR"spidev_resume\n");
    return 0;
}

int spidev_is_suspend(void)
{
    return g_spidev_suspend;
}
...
static struct of_device_id lattice_match_table[] = {
	{
		.compatible = "lattice,spidev",
	},
	{}
};
...
static struct spi_driver spidev_spi_driver = {
	.driver = {
		.name =		"spidev",
		.of_match_table = lattice_match_table,
		.owner =	THIS_MODULE,
		#ifdef CONFIG_IR_LATTICE_LED
		.pm=&spi_dev_pm_ops,
		#endif
