...
static bool floated_charger_enable = 1; /* ZTE_MODIFY yuanbo for floated charger */
...
/*++ ZTE: modify for eye pattern 20140303 ++*/
#if defined(CONFIG_PROJECT_P898S10_ZTE) || defined(CONFIG_PROJECT_P892S10_ZTE)
	if (!pdata->use_host_mode_param){
		ulpi_write( &motg->phy, ulpi_read( &motg->phy, 0x81) | 0xf, 0x81);
		ulpi_write( &motg->phy, 0x3b, 0x82);
	}
	printk(KERN_ERR "%s 0x81:0x%0x 0x82:0x%0x\n", __func__, ulpi_read( &motg->phy, 0x81), ulpi_read( &motg->phy, 0x82));
#endif
/*-- ZTE: modify for eye pattern 20140303 --*/
}
...
static void msm_otg_sm_work(struct work_struct *w)
{
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_FLOATED_CHARGER:
					msm_otg_notify_charger(motg,
							IDEV_CHG_MIN); /* ZTE_MODIFY yuanbo for floated charger */
					pm_runtime_put_noidle(otg->phy->dev);
					pm_runtime_suspend(otg->phy->dev);
					break;
	...
	case OTG_STATE_A_WAIT_VRISE:
		if ((test_bit(ID, &motg->inputs) &&
			...
			clear_bit(B_SESS_VLD, &motg->inputs);
			/*++ ZTE: modify for eye pattern in otg mode 20140406 ++*/
#if defined(CONFIG_PROJECT_P898S10_ZTE) || defined(CONFIG_PROJECT_P892S10_ZTE)
			motg->pdata->use_host_mode_param = true;
			msm_otg_reset(otg->phy);
			motg->pdata->use_host_mode_param = false;
			msleep(10);
#endif
			/*-- ZTE: modify for eye pattern in otg mode 20140406 --*/
			msm_otg_start_host(otg, 1);
