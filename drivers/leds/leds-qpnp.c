...
extern void set_led_breath_mode(int rgb,int enable);
extern void set_led_normal_on_mode(int rgb,int enable);
extern void set_led_normal_on_off_mode(int rgb,int enable);
#ifdef CONFIG_IR_LATTICE_LED_BREATH
extern void set_led_breath_mode_standard(int rgb,int enable);
extern void set_led_breath_mode_slow(int rgb,int enable);
#endif

static struct wakeup_source *ir_led_ws;
static int multi_enter_flag = 0;
#endif
...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
...
#ifdef CONFIG_IR_LATTICE_LED
    if (led->mpp_cfg->source_sel == 10 || led->mpp_cfg->source_sel == 8 || led->mpp_cfg->source_sel == 20 )
    {
           multi_enter_flag++;
           if ( multi_enter_flag == 1 )
           {
                 __pm_stay_awake(ir_led_ws);
           }
           printk(KERN_ERR"[led] qpnp_mpp_set source_sel=%d,brightness=%d\n",led->mpp_cfg->source_sel,led->cdev.brightness);
    }

    if (led->cdev.brightness)
    {
        if(10 == led->mpp_cfg->source_sel)  // red led
        {
             if (255 == led->cdev.brightness) // breath mode
            {
		  set_led_breath_mode(LED_RED_ON,1);	
            }
             #ifdef CONFIG_IR_LATTICE_LED_BREATH
             else if(100 ==led->cdev.brightness)
            {
                set_led_breath_mode_standard(LED_RED_ON,1);
            }
             else if(80 ==led->cdev.brightness)
            {
                set_led_breath_mode_slow(LED_RED_ON,1);
            }
             #endif
            else if(25 ==led->cdev.brightness)  //normal on off mode
            {
                set_led_normal_on_off_mode(LED_RED_ON,1);
            }
            else 
            {
                set_led_normal_on_mode(LED_RED_ON,1);	
            }
        }
        else if (8 == led->mpp_cfg->source_sel) // green led
        {
             if (255 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode(LED_GREEN_ON,1);	
            }
            #ifdef CONFIG_IR_LATTICE_LED_BREATH
            else  if (100 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode_standard(LED_GREEN_ON,1);
            }
             else if (80 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode_slow(LED_GREEN_ON,1);
            }
            #endif
            else if(25 ==led->cdev.brightness)  //normal on off mode
            {
                set_led_normal_on_off_mode(LED_GREEN_ON,1);
            }
            else 
            {
                set_led_normal_on_mode(LED_GREEN_ON,1);
            }
        
        }
        else if(20== led->mpp_cfg->source_sel) // blue led
        {
             if (255 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode(LED_BLUE_ON,1);	
            }
            #ifdef CONFIG_IR_LATTICE_LED_BREATH
            else  if (100 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode_standard(LED_BLUE_ON,1);
            }
            else  if (80 == led->cdev.brightness) // breath mode
            {
		set_led_breath_mode_slow(LED_BLUE_ON,1);
            }
            #endif
            else if(25 ==led->cdev.brightness)  //normal on off mode
            {
                set_led_normal_on_off_mode(LED_BLUE_ON,1);
            }
            else 
            {
                set_led_normal_on_mode(LED_BLUE_ON,1);	
            }
    ...
    else
    {
            if(10 == led->mpp_cfg->source_sel)  // red led
            {
                set_led_normal_on_off_mode(LED_RED_ON,0);
            }
            else if (8 == led->mpp_cfg->source_sel) // green led
            {
                set_led_normal_on_off_mode(LED_GREEN_ON,0);
            }
            else if(20== led->mpp_cfg->source_sel) // blue led
            {
                set_led_normal_on_off_mode(LED_BLUE_ON,0);
            }
     }
     ...
     if (led->mpp_cfg->source_sel == 10 || led->mpp_cfg->source_sel == 8 || led->mpp_cfg->source_sel == 20 )
     {
           ...
           multi_enter_flag--;
           if ( multi_enter_flag == 0 )
           {
                 __pm_relax(ir_led_ws);
           }
     }
     /*  ZTE_MODIFY: added by liyibo for button backlight at 2014-03-05 */
     if(1!= led->mpp_cfg->source_sel) // button-backlight
           return 0;
     /*  ZTE_MODIFY: added end  by liyibo */
 #endif
		...
		struct device_node *node)
{
	...
	if (!rc) {
			...
			if(strcmp(led->cdev.name,"led:rgb_blue") == 0)
				led->mpp_cfg->current_setting = 25;
		#else
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		if (rc < 0) {
			...
			else if(strcmp(led->cdev.name,"led:rgb_blue") == 0)
				{
				continue;
				}
		#endif
...
#ifdef CONFIG_IR_LATTICE_LED
	ir_led_ws=wakeup_source_register("ir_led");
#endif
	return 0;
...
#ifdef CONFIG_IR_LATTICE_LED
	wakeup_source_unregister(ir_led_ws);
#endif
	return 0;
}
