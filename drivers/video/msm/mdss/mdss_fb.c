...
static void mipi_dsi_check_live_status(struct work_struct *work);//qcomm lcd esd patch start
static void mipi_dsi_reset_display(struct work_struct *work);//qcomm lcd esd patch start
extern int read_the_status(struct mdss_dsi_ctrl_pdata *ctrl);
//lipeng add for esd test 20140429--
#endif
...
#if defined CONFIG_PROJECT_P892S10_ZTE
//lipeng add for esd test 20140429++
	INIT_DELAYED_WORK(&mfd->panel_live_status, mipi_dsi_check_live_status);//qcomm lcd esd patch start
	INIT_WORK(&mfd->display_reset, mipi_dsi_reset_display);//qcomm lcd esd patch start
//lipeng add for esd test 20140429--
#endif
	return rc;
}
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if (((!mfd->panel_power_on && mfd->dcm_state != DCM_ENTER)
		|| !mfd->bl_updated || !is_splash_finish) && !IS_CALIB_MODE_BL(mfd)) {
...
{
	struct mdss_panel_data *pdata;
	//pr_info("is_splash_finish = %d\n", is_splash_finish);
	if (mfd->unset_bl_level && !mfd->bl_updated && is_splash_finish) {
		pdata = dev_get_platdata(&mfd->pdev->dev);
		if ((pdata) && (pdata->set_backlight)) {
			mutex_lock(&mfd->bl_lock);
...
//	down(&mfd->sem);
	...
	if (pdata && pdata->set_backlight)
		pdata->set_backlight(pdata, mfd->bl_level);
//	up(&mfd->sem);
	printk("lipeng:mdss_fb_blank_sub start\n");
...
//	mdp_dma_pan_update(mfd->fbi);
	printk("lipeng:mdss_fb_blank_sub end\n");
//	down(&mfd->sem);
...
//	int read_the_status(struct mdss_dsi_ctrl_pdata *ctrl);
	...
	pdata = dev_get_platdata(&mfd->pdev->dev); 
	if(pdata->panel_info.type != MIPI_VIDEO_PANEL && pdata->panel_info.type != MIPI_CMD_PANEL )
	{
		printk("lipeng %s pdata->panel_info.type=%d\n", __func__, pdata->panel_info.type);
		return;
	}
	ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	
	read_the_status(ctrl);
	printk("lipeng %s is_panel_alive=%d\n", __func__, ctrl->is_panel_alive);
	if (ctrl->is_panel_alive == false)   //false
		schedule_work(&mfd->display_reset);

	if (mfd->panel_power_on)
		schedule_delayed_work(&mfd->panel_live_status, msecs_to_jiffies(4000));
}
//qcom lcd esd patch end
//lipeng add for esd test 20140429--
#endif
static int mdss_fb_blank_sub(int blank_mode, struct fb_info *info,
			     int op_enable)
{
	...
	printk("lipeng %s pdata->panel_info.type=%d blank_mode %d index = %d \n", __func__, pdata->panel_info.type, blank_mode, mfd->index);
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on && mfd->mdp.on_fnc) {
			...
			if (ret == 0) {
				...
				if(pdata->panel_info.type == MIPI_VIDEO_PANEL || pdata->panel_info.type == MIPI_CMD_PANEL )	
				{
					ctrl->is_panel_alive = true;//qcomm lcd esd patch start
				}

				if(initflag == 0)
				{
					schedule_delayed_work(&mfd->panel_live_status, msecs_to_jiffies(100000));//qcomm lcd esd patch 
					initflag = 1;
				}
				else
					schedule_delayed_work(&mfd->panel_live_status, msecs_to_jiffies(5000));//qcomm lcd esd patch 
				//lipeng add for esd test 20140429--
...
#if defined CONFIG_PROJECT_P892S10_ZTE
            //lipeng add for esd test 20140429++
			cancel_delayed_work_sync(&mfd->panel_live_status);//qcomm lcd esd patch 
