...
/* modify begin for rear camera imx135 by chengjia 2013-12-17 */
...
/*modify end by chengjia*/
//////////////////////////////chengjia added for ap1302
if (of_property_read_bool(of_node, "qcom,gpio-ispxvdd") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-ispxvdd", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-ispxvdd failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ispxvdd invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_ISP_XVDD] =
			gpio_array[val];
		}
if (of_property_read_bool(of_node, "qcom,gpio-ispiovdd") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-ispiovdd", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-ispiovdd failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ispiovdd invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_ISP_IOVDD] =
			gpio_array[val];
		}
/** ZTE_MODIFY by zhangxiaobo to use new power control for new 898S10 board 2014-03-11 */
if (of_property_read_bool(of_node, "qcom,gpio-ispiovddhmisc") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-ispiovddhmisc", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-ispiovddhmisc failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-ispiovddhmisc invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_ISP_IOVDD_HMISC] =
			gpio_array[val];
		}
/** ZTE_MODIFY end*/
if (of_property_read_bool(of_node, "qcom,gpio-flash-int-898") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-flash-int-898", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-flash-int-898 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-flash-int-898 invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_FLASH_INT_898] =
			gpio_array[val];
		}
if (of_property_read_bool(of_node, "qcom,gpio-flash-hwen-898") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-flash-hwen-898", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-flash-hwen-898 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-flash-hwen-898 invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_FLASH_HWEN_898] =
			gpio_array[val];
		}
if (of_property_read_bool(of_node, "qcom,gpio-flash-hwen-892") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-flash-hwen-892", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-flash-hwen-892 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-flash-hwen-892 invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_FLASH_HWEN_892] =
			gpio_array[val];
		}
/////////////////////////
/*modify end by chengjia*/
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	CDBG("%s:%d\n", __func__, __LINE__);
///chengjia modified for ap1302
	if(strcmp("ap1302", s_ctrl->sensordata->sensor_name)) {
		
	
	s_ctrl->func_tbl->sensor_power_down(s_ctrl);
		}
///chengjia modified end
