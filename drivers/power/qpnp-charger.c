...
#define GPIO_EXT_OVP_EN (109)

#define SPEED_99P_CHG_TIME
static int batttemp_count = 1;
struct qpnp_chg_irq {
	int		irq;
...
struct qpnp_chg_chip	*the_qpnp_chg_chip;

static bool flag_gpio_ext_ovp_usable = 0;
static int iusb_control = 1500;
...
static char *batt_status_text[] = {
	"Unknown", "Charging", "Discharging", "Not charging", "Full"
};

static char *charge_stage[] = {
...
static int control_ext_ovp(bool enable);
...
int g_usb_present = -1;
/*
 * Get the cmdline value
 */
static int __init ftm_mode_setup(char *str)
{
    if (!strncmp(str, "true", sizeof(true)))
    {
         ftm_mode = 1;
    }

    printk(KERN_NOTICE "@@ %s ftm_mode=%d\n", __func__, ftm_mode);
    return 1;
}
__setup("zte.ftmmod=", ftm_mode_setup);
...
qpnp_chg_is_batt_temp_ok(struct qpnp_chg_chip *chip)
{
	...
	if(en_no_batt == 1){
		pr_err("batt_rt_temp= %ld,return 1\n",(batt_rt_sts & BAT_TEMP_OK_IRQ));
		return 1;
	}else
...
{
	u8 usbin_chg_rt_sts, usb_chgpth_rt_sts, usbin_health = 0;
	int rc;
...
static int
qpnp_chg_force_usb_release(struct qpnp_chg_chip *chip)
{
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	if ((qpnp_chg_is_usb_chg_plugged_in(chip)
		...
		if(en_no_batt == 1)
              qpnp_chg_force_usb_release(chip);
		else 
						...
						enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
       case POWER_SUPPLY_PROP_IBAT_MAX:
       case POWER_SUPPLY_PROP_IUSB_CONTROL:
		return 1;
	default:
		break;
	}
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	//POWER_SUPPLY_PROP_IBAT_MAX,
	POWER_SUPPLY_PROP_IUSB_CONTROL,
};
...
static int en_no_batt_set(const char *val, const struct kernel_param *kp)
{
	int rc;
	struct power_supply *batt_psy = power_supply_get_by_name("battery");
	struct qpnp_chg_chip *chip = container_of(batt_psy, struct qpnp_chg_chip,
								batt_psy);
	...
	if (*(bool *)kp->arg) {
		qpnp_chg_disable_irq(&chip->batt_pres);
		qpnp_chg_disable_irq(&chip->batt_temp_ok);
		disable_irq_wake(chip->batt_pres.irq);
		disable_irq_wake(chip->batt_temp_ok.irq);
		pr_err("disable batt_pres&batt_temp_ok_irq: %d\n", rc);
	}else{
		qpnp_chg_enable_irq(&chip->batt_pres);
		qpnp_chg_enable_irq(&chip->batt_temp_ok);
		enable_irq_wake(chip->batt_pres.irq);
		enable_irq_wake(chip->batt_temp_ok.irq);
		pr_err("enable batt_pres&batt_temp_ok_irq: %d\n", rc);
	}
...
static struct kernel_param_ops en_no_batt_ops = {
	.set = en_no_batt_set,
	.get = param_get_bool,
};

module_param_cb(en_no_batt, &en_no_batt_ops, &en_no_batt, 0600);
...
get_prop_batt_present(struct qpnp_chg_chip *chip)
{
	...
	if(en_no_batt == 1){
		pr_info("batt_present= %ld,return 1\n",(batt_present & BATT_PRES_BIT));
		return 1;
	}else		
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		printk(KERN_ERR"%s: No BMS supply registered return 50\n", __func__);
	}
...
/* jiangzhifeng add for LC709203 fuel gauge */
int g_batt_temp = -1;
static int
get_prop_batt_temp(struct qpnp_chg_chip *chip)
{
	...
	if(en_no_batt == 1){
		pr_info("get_bat_temp: %d\n",(int)(results.physical));
		return (DEFAULT_TEMP - 10);
	}else{
       /* jiangzhifeng add for LC709203 fuel gauge */
       g_batt_temp = results.physical;

	   if(((int)results.physical >= 700)&&(batttemp_count < 6))
	   {	
	   		results.physical = 555;
			batttemp_count++;
	   }else{
			batttemp_count = 1;
	   }
		...
		if (ret.intval <= 2 && !chip->use_default_batt_values &&
			...
			qpnp_chg_iusbmax_set(chip, QPNP_CHG_I_MAX_MIN_100);
		       ...
		       vbat_uv = get_prop_battery_voltage_now(chip);
                     if ( ftm_mode == 1 && get_prop_batt_present(chip) == 0)
                     {
                           printk(KERN_ERR" %s usb_suspend set 1 in ftm mode, vbat_uv=%d\n", __func__, vbat_uv);
                           qpnp_chg_usb_suspend_enable(chip, 1);
                     }
                     else
                     {
...
static void
qpnp_chg_set_appropriate_iusb_current(struct qpnp_chg_chip *chip, unsigned int mA)
{
       mA = min(mA, the_msm_otg->current_max/1000);

	printk(KERN_ERR"current_max =%d, set= %d mA\n", the_msm_otg->current_max/1000, mA);
...
qpnp_eoc_work(struct work_struct *work)
{
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		{
                       #ifdef SPEED_99P_CHG_TIME
                       if(get_prop_capacity(chip) == 99)
                       {
                               /* eco work loop every 10s, 5min=30 times */
                               if ( cv_count == 30 )
                                      chip->max_voltage_mv = 4340;
                               else
                                       cv_count++;
                       }
                       else if(chip->max_voltage_mv != 4360)
                       {
                               chip->max_voltage_mv = 4360;
                               cv_count = 0;
                       }
                       pr_info("2.CV max_voltage: %d, cv_count=%d\n",(chip->max_voltage_mv), cv_count);
                       #endif
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
       ...
       case POWER_SUPPLY_PROP_IBAT_MAX:
              if (val->intval < 50)
              {
                    printk(KERN_ERR"%s set IBAT_MAX val=%d, <50mA, stop charging!\n", __func__, val->intval);
                    qpnp_chg_buck_control(chip, 0);
              }
              else
              {
                    printk(KERN_ERR"%s set IBAT_MAX mA=%d\n", __func__, val->intval);
                    qpnp_chg_buck_control(chip, 1);
                    qpnp_chg_ibatmax_set(chip, val->intval);
              }
		break;
       case POWER_SUPPLY_PROP_IUSB_CONTROL:
              printk(KERN_ERR"%s iusb control =%d, now set =%d\n", __func__, iusb_control, val->intval);
              iusb_control = val->intval;
              qpnp_chg_set_appropriate_iusb_current(chip, val->intval);
		break;
	default:
		return -EINVAL;
	}
...
{
/*  qcom,battery-data configuration in msm8226-qrd.dtsi will overwrite the settings in msm-pm8226.dtsi, we use the latter.
				...
				struct spmi_resource *spmi_resource)
{
	...
	case SMBCL_BAT_IF_SUBTYPE:
        rc = qpnp_chg_masked_write(chip,
                chip->bat_if_base + CHGR_BAT_IF_BATFET_CTRL4,
                BATFET_LPM_MASK,      //just write bit[6:7]
                BATFET_NO_LPM,1);		
...
/* enlarge charging current */
static int control_ext_ovp(bool enable)
{
    if ( flag_gpio_ext_ovp_usable )
    {
       if ( enable == true)
       {
            printk(KERN_ERR"%s  enable\n", __func__);
	    gpio_direction_output(GPIO_EXT_OVP_EN, 1);
       }
       else
       {
            printk(KERN_ERR"%s  disable\n", __func__);
            gpio_direction_output(GPIO_EXT_OVP_EN, 0);
       }
       return 0;
    }
    printk(KERN_ERR"%s ########## gpio 109 can not use\n", __func__);
    return -1;
}

void charge_status_debug(void)
{
    struct qpnp_chg_chip	*chip= the_qpnp_chg_chip;
    
    printk(KERN_ERR"%s: batt_present=%d, batt_online=%d, batt_temp=%d, batt_status=%s, usb_present=%d, usb_online=%d, usb_type=%s, health=%s, charge_stage=%s\n", 
                                __func__,    chip->batt_present,   qpnp_chg_is_batfet_closed(chip),   
                                g_batt_temp,  batt_status_text[get_prop_batt_status(chip)],   chip->usb_present, 
                                the_msm_otg->online,   usb_type_text[chip->usb_psy->type], 
                                health_text[get_prop_batt_health(chip)],   charge_stage[get_prop_charge_type(chip)]);
}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	ret = gpio_request(GPIO_EXT_OVP_EN, "EXT_OVP_EN");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__, GPIO_EXT_OVP_EN);
	}
       else
       {
              pr_info("%s: request gpio %d success, disable ovp at power on\n", __func__, GPIO_EXT_OVP_EN);
              flag_gpio_ext_ovp_usable = 1;
              control_ext_ovp(false);
       }

       the_qpnp_chg_chip = chip;
	...
	wake_lock_init(&charger_wake_lock, WAKE_LOCK_SUSPEND, "charger_wake_lock");
