...
extern void charge_status_debug(void);
			...
			u16 base, int count)
{
	...
	if (rc) {
		pr_err("SPMI read failed rc=%d, try 2 times again!\n", rc);
              msleep(10);
              rc = spmi_ext_register_readl(spmi->ctrl, spmi->sid, base, val, count);
              if (rc)
              {
				...
				int batt_temp)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		printk(KERN_ERR"%s: PON_OCV_UV = %d, cc = %llx\n",
				__func__, chip->last_ocv_uv, raw->cc);
...
out:
	printk(KERN_ERR"%s: ibat_ua = %d, vbat_uv = %d, ocv_est_uv = %d, SOC = %d, last_SOC = %d, pc_est = %d, soc_est = %d, n = %d, delta_ocv_uv = %d, last_ocv_uv = %d, pc_new = %d, soc_new = %d, rbatt = %d, slope = %d\n",
		__func__, ibat_ua, vbat_uv, ocv_est_uv, soc, chip->last_soc, pc_est,
		soc_est, n, delta_ocv_uv, chip->last_ocv_uv,
		pc_new, soc_new, params->rbatt_mohm, slope);

#ifdef RAISE_BATT_CAPACITY
       show_capacity = trans_real_to_show_cap(chip,get_prop_bms_capacity(chip));
       printk(KERN_ERR"fix batt capacity=%d,  SOC=%d\n", show_capacity, soc);
#endif
...
static int recalculate_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->use_voltage_soc) {
		...
		if (rc) {
			...
			if (batt_temp >= 700)
                     {
                            pr_debug("batt_temp is over 70C , to power down\n");
                            orderly_poweroff(true);
                     }

                     get_battery_voltage(chip, &batt_voltage_uv);

                     if ( (ftm_mode == 1) && (is_battery_present(chip)) &&  (batt_voltage_uv < 3400000) && ( g_usb_present == 0) )
                     {
                            pr_info("in FTM, vbat<3.4v , to power down\n");
                            //orderly_poweroff(true);
                            emergency_sync();
		              kernel_power_off();
                     }
