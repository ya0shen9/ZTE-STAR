...
static struct delayed_work lc709203_set_temp_work;
static struct delayed_work lc709203_get_capcity_work;
...
void  lc709203_write_battery_temp_work(struct work_struct *work)
{
    u16 temperature=0;
    ...
    if(g_lc709203_driver_init_done)
    {
        ...
        if (g_batt_temp != batt_temp_last)
        {
            temperature = g_batt_temp+LC709203_TEMP_OFFSET;
            pr_info("%s: batt temp = %d, write reg[0x%x] = 0x%x\n",  __func__, g_batt_temp, CELL_TEMP_ADDR, temperature);
            
            lc709203_write_word(CELL_TEMP_ADDR, temperature);
        }

        batt_temp_last = g_batt_temp;
    }
    else
    {
    ...
    schedule_delayed_work(&lc709203_set_temp_work, msecs_to_jiffies(1000));
}
...
int lc709203_power_on_init(void)
{
    int ret = -1;
    ...
    schedule_delayed_work(&lc709203_set_temp_work, msecs_to_jiffies(1000));
    schedule_delayed_work(&lc709203_get_capcity_work, msecs_to_jiffies(3000));
...
u16 lc709203_get_capacity(void)
{
    ...
    printk(KERN_ERR"LC709203: batt temp=%d, vbat=%d, RSOC=%d, ITE=%d, Corrected capcity=%d\n", 
                               reg_temp-2732, reg_volt, rsoc, ite, corrected_capcity); 

    return corrected_capcity;
}

void lc709203_get_capacity_work(struct work_struct *work)
{
    lc709203_get_capacity();
    schedule_delayed_work(&lc709203_get_capcity_work, msecs_to_jiffies(10000));
}
...
{             
    ...
    INIT_DELAYED_WORK(&lc709203_set_temp_work, lc709203_write_battery_temp_work);
    INIT_DELAYED_WORK(&lc709203_get_capcity_work, lc709203_get_capacity_work);
    
    // lc709203_dump_register();
    lc709203_power_on_init();
