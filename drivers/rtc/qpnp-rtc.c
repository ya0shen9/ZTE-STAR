...
static irqreturn_t qpnp_alarm_trigger(int irq, void *dev_id)
{
	...
	unsigned long irq_flags;
        ...
        struct rtc_time rtc_tm;//ADD THIS 
        struct rtc_wkalrm alarm;//ADD THIS 
        u8 rtc_alrm_rt_sts, rtc_alrm_type, rtc_alrm_pol_hi, rtc_alrm_pol_lo;//ADD THIS 

        /* ADD THE FOLLOWING DEBUG CODE */ 
        rc = qpnp_rtc_read_time(alarm_rtc_dev->dev.parent, &rtc_tm); 
        if (rc) { 
        dev_err(alarm_rtc_dev->dev.parent, "Unable to read RTC time\n"); 
        return -EINVAL; 
        } 
        printk(KERN_ERR"qpnp_alarm_trigger: RTC TIME: h:m:s=%d:%d:%d, d/m/y=%d/%d/%d\n", 
        rtc_tm.tm_hour, rtc_tm.tm_min, 
        rtc_tm.tm_sec, rtc_tm.tm_mday, 
        rtc_tm.tm_mon, rtc_tm.tm_year); 

        rc = qpnp_rtc_read_alarm(alarm_rtc_dev->dev.parent, &alarm); 
        if (rc) { 
        dev_err(alarm_rtc_dev->dev.parent, "Unable to read RTC ALARM\n"); 
        return -EINVAL; 
        } 
        printk(KERN_ERR"qpnp_alarm_trigger: Alarm Set for h:m:s=%d:%d:%d, d/m/y=%d/%d/%d\n", 
        alarm.time.tm_hour, alarm.time.tm_min, 
        alarm.time.tm_sec, alarm.time.tm_mday, 
        alarm.time.tm_mon, alarm.time.tm_year); 

        rc = qpnp_read_wrapper(rtc_dd, &rtc_alrm_rt_sts, 
        rtc_dd->alarm_base + 0x10, 1); 
        printk(KERN_ERR"qpnp_alarm_trigger: RTC ALRM RT STS: 0x%X\n", rtc_alrm_rt_sts); 

        rc = qpnp_read_wrapper(rtc_dd, &rtc_alrm_type, 
        rtc_dd->alarm_base + 0x11, 1); 
        printk(KERN_ERR"qpnp_alarm_trigger: RTC ALRM TYPE: 0x%X\n", rtc_alrm_type); 

        rc = qpnp_read_wrapper(rtc_dd, &rtc_alrm_pol_hi, 
        rtc_dd->alarm_base + 0x12, 1); 
        printk(KERN_ERR"qpnp_alarm_trigger: RTC ALRM POL HI: 0x%X\n", rtc_alrm_pol_hi); 

        rc = qpnp_read_wrapper(rtc_dd, &rtc_alrm_pol_lo, 
        rtc_dd->alarm_base + 0x13, 1); 
        printk(KERN_ERR"qpnp_alarm_trigger: RTC ALRM POL LO: 0x%X\n", rtc_alrm_pol_lo); 
#endif
	printk(KERN_ERR"qpnp_alarm_trigger \n");
