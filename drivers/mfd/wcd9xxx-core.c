...
/* ZTE_modif add for TFA9890 reset begin */
#ifdef CONFIG_SND_SOC_TFA9890
static int tfa9890_reset(struct device *dev)
{
	int reset_gpio;
        int ret;

        
	reset_gpio = of_get_named_gpio(dev->of_node,
				"qcom,tfa9890-reset-gpio", 0);
     
        dev_info(dev, "%s() reset_gpio=%d\n", __func__, reset_gpio);
   
	if (reset_gpio < 0) {
		dev_err(dev, "Looking up %s property in node %s failed %d\n",
			"qcom, tfa9890-reset-gpio", dev->of_node->full_name, reset_gpio);
		return -ENXIO;
	}    
     
	ret = gpio_request(reset_gpio, "TFA9890 RESET");
	if (ret) {
...
static ssize_t tfa9890_reset_set(struct device *dev,
...
static DEVICE_ATTR(tfa9890_reset, 0664,
		   NULL, tfa9890_reset_set);

static struct attribute *tfa9890_sysfs_attrs[] = {
	&dev_attr_tfa9890_reset.attr,
	NULL
};

static struct attribute_group tfa9890_core_sysfs = {
	.attrs = tfa9890_sysfs_attrs
};
#endif
/* ZTE_modif add for TFA9890 reset end */
...
static const struct intr_data intr_tbl_v2[] = {
	...
	{WCD9XXX_IRQ_HPH_L_PA_STARTUP, false},
	{WCD9XXX_IRQ_HPH_R_PA_STARTUP, false},
	//{WCD9320_IRQ_EAR_PA_STARTUP, false},   /* ZTE_Modify for headset false detect by lvrongguo 20140317 */
	{WCD9XXX_IRQ_RESERVED_0, false},
	{WCD9XXX_IRQ_RESERVED_1, false},
...
static struct wcd9xxx_pdata *wcd9xxx_populate_dt_pdata(struct device *dev)
{
	...
	const char *cp_supplies_name = "qcom,cdc-cp-supplies";
        ...
        int es_supplies_cnt;
        const char *es_supplies_name = "qcom,cdc-es704-supplies";
        #endif
	...
	if (IS_ERR_VALUE(cp_supplies_cnt))
        ...
        #ifdef CONFIG_SND_SOC_ES705   
	es_supplies_cnt = of_property_count_strings(dev->of_node,
							es_supplies_name);
	if (IS_ERR_VALUE(es_supplies_cnt))
		es_supplies_cnt = 0;
        #endif
        
	BUG_ON(static_cnt <= 0 || ond_cnt < 0 || cp_supplies_cnt < 0);
        #ifdef CONFIG_SND_SOC_ES705
	if ((static_cnt + ond_cnt + cp_supplies_cnt + es_supplies_cnt)
        #else
	...
	if (ret)
        ...
        #ifdef CONFIG_SND_SOC_ES705
	ret = wcd9xxx_process_supplies(dev, pdata, es_supplies_name,
				es_supplies_cnt, STATIC_REGULATOR, static_cnt + ond_cnt+cp_supplies_cnt);
        #endif
...
static int wcd9xxx_slim_probe(struct slim_device *slim)
{
	...
	/* ZTE_modif add for TFA9890 reset begin */
	#ifdef CONFIG_SND_SOC_TFA9890
	tfa9890_reset(&slim->dev);

	ret = sysfs_create_group(&slim->dev.kobj, &tfa9890_core_sysfs);
	if (ret) {
		pr_err("%s: Fail to set tfa9890 reset sysfs\n",
		       __func__);
	}        
	#endif
	/* ZTE_modif add for TFA9890 reset end */	
