...
/* ZTE_MODIFY : modify begin by liyibo for the the irq gpio 17 to pull none 2014-05-23*/
struct gpiomux_setting goodix_int_act_cfg = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};

static struct msm_gpiomux_config msm_goodix_configs[] = {
       {
		.gpio = 17,
		.settings = {
...
/* ZTE_MODIFY : modify end by liyibo for the the irq gpio 17 to pull none 2014-05-23*/
...
/* ZTE_MODIFY : modify begin by liyibo for project cmd 2014-05-30*/
static int goodix_info_proc_show(struct seq_file *m, void *v)
{
	int len = 0;
	char buf[256];

	len += sprintf(buf + len, "Name: Goodix\n");
	len += sprintf(buf + len, "I2C address: 0x%x\n", 0x14);
	len += sprintf(buf + len, "IC type: GT615\n");
	len += sprintf(buf + len, "Module : Yushun\n");
...
static int goodix_info_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, goodix_info_proc_show, NULL);
}

static const struct file_operations goodix_info_proc_fops = {
	.open		= goodix_info_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int goodix_info_proc_init(void)
{
...
/* ZTE_MODIFY : modify end by liyibo for project cmd 2014-05-30*/
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (ret) {
       ...
       /* ZTE_MODIFY : modify by liyibo for the the irq gpio 17 to pull none 2014-05-23*/
       msm_gpiomux_install(msm_goodix_configs, ARRAY_SIZE(msm_goodix_configs));
	...
	if (ret != 0) {
       ...
       goodix_info_proc_init();   /* ZTE_MODIFY : modify by liyibo for project cmd 2014-05-30*/
