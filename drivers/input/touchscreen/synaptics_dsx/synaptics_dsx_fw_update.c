...
#define FW_IMAGE_NAME_TRULY "ZTE_P682A10_Truly_PR1237913-s3202_34323034_GloveOff_1222.img"
#define FW_IMAGE_NAME_ECW "ZTE_P862A10_ECW_PR1237913-s3202_34393034_GloveOff_1222.img"
#define FW_IMAGE_NAME_EACHOPTO "ZTE_P862A10_eachopto_PR1237913-s3202_34433034_GloveOff_1222.img"
#define FW_IMAGE_NAME_JUNDA "ZTE_P689A10_Junda_PR1237913-s3202Glove_34453039_noGlove_1222.img"
#define FW_IMAGE_NAME_DEFAULT "startup_fw_update.img"
#ifdef CONFIG_PROJECT_P898S10_ZTE
#define DSX_IMAGE_NAME_TPK	 		"ZTE_P692S10_PR1631522-s3202_TPK_34313050.img"
#define DSX_IMAGE_NAME_GOWORLD		"ZTE_P692S10_PR1631522-s3202_Goworld_34413048.img"
#define DSX_IMAGE_NAME_LAIBAO              "ZTE_P692S10_PR1631522-s3202_Laibao_34373032.img"
#endif

#ifdef CONFIG_PROJECT_P892S10_ZTE
#define DSX_IMAGE_NAME_TIANMA     "TIANMA_P892S10_PR1700428-dsx6653_47473056.img"
#define DSX_IMAGE_NAME_JINGDONGFANG     "ZTE_P892S10_PR1529907_2316_BOE_5inch_DS4_3.5.5_47463034.img"
#endif
/*
...
#else
//#ifdef CONFIG_PROJECT_P898S10_ZTE
       printk("---tp--- %s : image_fw_id =%d ,device_fw_id = %d\n" , __func__, (int)image_fw_id,  device_fw_id);
	if (image_fw_id != device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	}
//#endif
	...
	dev_info(rmi4_data->pdev->dev.parent,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
       dev_info(rmi4_data->pdev->dev.parent,"%s image_config_id = %d, device_config_id = %d\n" , 
	   	__func__, image_config_id , device_config_id);
  if(config_id[1] == 0x30 && config_id[1] == 0x30 &&  config_id[2] == 0x30 &&  config_id[3] == 0x38 ) 
  {
...
static int fwu_start_reflash(void)
{
	...
	pr_info("firmware_information is 0x%x, 0x%x, 0x%x, 0x%x\n", firmware_information[0], firmware_information[1], firmware_information[2], firmware_information[3]);
	...
	if (fwu->ext_data_source) {
		...
		switch (firmware_information[1]){
			case 0x31:
				#ifdef CONFIG_PROJECT_P898S10_ZTE
				strncpy(fwu->image_name, DSX_IMAGE_NAME_TPK,MAX_IMAGE_NAME_LEN);
				#endif
				break;
		 	case 0x32:			
				strncpy(fwu->image_name, FW_IMAGE_NAME_TRULY, MAX_IMAGE_NAME_LEN);			
				break ;
			case 0x37:
				#ifdef CONFIG_PROJECT_P898S10_ZTE
				strncpy(fwu->image_name, DSX_IMAGE_NAME_LAIBAO, MAX_IMAGE_NAME_LEN);
				#endif
			case 0x39:			
				strncpy(fwu->image_name, FW_IMAGE_NAME_ECW, MAX_IMAGE_NAME_LEN);			
				break ;
			case 0x43:			
				strncpy(fwu->image_name, FW_IMAGE_NAME_EACHOPTO, MAX_IMAGE_NAME_LEN);			
				break ;	
			case 0x45:			
				strncpy(fwu->image_name, FW_IMAGE_NAME_JUNDA, MAX_IMAGE_NAME_LEN);	
	              case 0x41:
				#ifdef CONFIG_PROJECT_P898S10_ZTE
				strncpy(fwu->image_name, DSX_IMAGE_NAME_GOWORLD, MAX_IMAGE_NAME_LEN);
				#endif
			       break;
			case 0x30:
			case 0x46:	
				#ifdef CONFIG_PROJECT_P892S10_ZTE
				strncpy(fwu->image_name, DSX_IMAGE_NAME_JINGDONGFANG, MAX_IMAGE_NAME_LEN);
				#endif
				break;
			case 0x47:
				#ifdef CONFIG_PROJECT_P892S10_ZTE
				strncpy(fwu->image_name, DSX_IMAGE_NAME_TIANMA, MAX_IMAGE_NAME_LEN);
				#endif
				break;
			default:
				strncpy(fwu->image_name, FW_IMAGE_NAME_DEFAULT, MAX_IMAGE_NAME_LEN);
				break ;
		}
...
exit:
	...
	rmi4_data->stay_awake = false;
       ...
       fwu_scan_pdt();   /* added by liyibo for update version No */
...
static int TP_info_proc_show(struct seq_file *m, void *v)
{
	...
	#ifdef CONFIG_PROJECT_P892S10_ZTE
	len += sprintf(buf + len, "IC type: S2316\n");
	#endif

       printk("---tp--- : 0x%x, 0x%x, 0x%x, 0x%x\n", firmware_information[0],firmware_information[1],firmware_information[2],firmware_information[3]);
	...
	case 0x32:
		...
		break ;
	case 0x33:
	 	len += sprintf(buf + len, "Module : Success\n");
		break ;
	case 0x34:
	 	len += sprintf(buf + len, "Module : Oflim\n");
		break ;
	case 0x35:
	 	len += sprintf(buf + len, "Module : Lead\n");
		break ;
	case 0x36:
	 	len += sprintf(buf + len, "Module : Wintek\n");
		break ;
	case 0x37:
	 	len += sprintf(buf + len, "Module : Laibao\n");
		break ;
	case 0x38:
	 	len += sprintf(buf + len, "Module : CMI\n");
		break ;
	case 0x39:
	 	len += sprintf(buf + len, "Module : Eely\n");
		break ;
	case 0x41:
	 	len += sprintf(buf + len, "Module : Goworld\n");
		break ;
	case 0x42:
	 	len += sprintf(buf + len, "Module : Baoming\n");
		break ;
	case 0x43:
	 	len += sprintf(buf + len, "Module : Eachopto\n");
		break ;
	case 0x44:
	 	len += sprintf(buf + len, "Module : Mutto\n");
		break ;
	case 0x45:
	 	len += sprintf(buf + len, "Module : Junda\n");
		break ;
	case 0x30:
	case 0x46:
		len += sprintf(buf + len, "Module : JingDongFang\n");
		break ;
	case 0x47:
		len += sprintf(buf + len, "Module : Tianma\n");
		break;
	default:
