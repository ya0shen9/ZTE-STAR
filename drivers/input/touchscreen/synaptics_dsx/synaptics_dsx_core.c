		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!bdata->cap_button_map) {
		...
		f1a->button_map[0]=KEY_BACK;
		f1a->button_map[1]=KEY_HOMEPAGE;
		f1a->button_map[2]=KEY_MENU;
	}
...
static void syna_init_vkeys(void)
{
	int rc = 0;
...
#endif
#if 0
static u8 current_gloved_finger_state = 0;
struct synaptics_rmi4_data *g_rmi4_data = NULL;
static int gloved_finger_proc_show(struct seq_file *m, void *v)
{
		
	seq_printf(m, "%d\n", current_gloved_finger_state); 
	 printk("=== Syna [mt_tpd_read]%s ,line=%d\n", __func__,__LINE__);	
	return 0;
}
...
{
	unsigned char tempvalue = 0, tempvalue1 = 0;
	int t0, t1;
	current_gloved_finger_state = value;

	synaptics_rmi4_reg_read(g_rmi4_data, 0x400,  &tempvalue, 1);
	if (value == 1)
	{
		synaptics_rmi4_reg_write(g_rmi4_data, 0x400,  &value, 1);
	}
	else if (value == 0) 
	{
		value = 2;
	...
	synaptics_rmi4_reg_read(g_rmi4_data, 0x400,  &tempvalue1, 1);
       t0 = tempvalue;
	t1 = tempvalue1;
         printk("=== Syna [mt_tpd_read]%s ,line=%d,current_gloved_finger_state=0x%x, temp %d %d\n", __func__,__LINE__,current_gloved_finger_state, t0, t1);
...
{	
	...
	unsigned char tempvalue = 0, tempvalue1 = 0;
	int t0, t1;
         printk("=== Syna [mt_tpd_read]%s ,line=%d,count=0x%x,\n", __func__,__LINE__,count);
	...
	for (; i < count; i++)
	{
		...
		if (rc)
		return rc;
	       printk("=== Syna [mt_tpd_read]%s ,line=%d,0x%x,\n", __func__,__LINE__,ret);
	...
	if (ret == 1)
	{
		synaptics_rmi4_reg_write(g_rmi4_data, 0x400,  &ret, 1);
	}
	else if (ret == 0) 
	{
		ret = 2;
...
static int gloved_finger_proc_init(void)
{
	...
	res = create_proc_entry("gloved_finger_switch", S_IWUGO | S_IRUGO, NULL);
	if (!res)
	{
		printk(KERN_INFO "failed to create /proc/gloved_finger_switch\n");
		return 0;
	}
	res->proc_fops = &gloved_finger_proc_fops;
	printk(KERN_INFO "created /proc/gloved_finger_switch\n");
	return 0;
}
#endif
#ifdef CONFIG_PROJECT_P892S10_ZTE_TEST
int synaptics_suspend_flag = 0;
static void synaptics_rmi4_resume_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;
	pr_info("lip touch %s : synaptics_rmi4_resume\n", __func__);
	synaptics_rmi4_resume(&(rmi4_data->input_dev->dev));
	synaptics_suspend_flag = 0;
	return;
}
#endif
 /**
 ...
 * modules.
 */
/* added begin by liyibo for doze mode at 2014-06-25 */
struct synaptics_rmi4_data *g_rmi4_data = NULL;

static int  finger_proc_write(void)
{	
	char ret;
	unsigned char tempvalue = 0, tempvalue1 = 0,tempvalue2 = 0, tempvalue3 = 0;
	int t0, t1,t2,t3;

       printk("=== Syna [mt_tpd_read]%s ,line=%d\n", __func__,__LINE__);

	synaptics_rmi4_reg_read(g_rmi4_data, 0x0053,  &tempvalue, 1);
	synaptics_rmi4_reg_read(g_rmi4_data, 0x0054,  &tempvalue1, 1);

       ret = 0x2D;
	synaptics_rmi4_reg_write(g_rmi4_data, 0x0053,  &ret, 1);

	ret = 0x05;
	synaptics_rmi4_reg_write(g_rmi4_data, 0x0054,  &ret, 1);
	
       synaptics_rmi4_reg_read(g_rmi4_data, 0x0053,  &tempvalue2, 1);
	synaptics_rmi4_reg_read(g_rmi4_data, 0x0054,  &tempvalue3, 1);
       t0 = tempvalue;
	t1 = tempvalue1;
	t2 = tempvalue2;
	t3 = tempvalue3;
       printk("=== Syna [mt_tpd_read]%s ,line=%d, temp %d %d %d %d\n", __func__,__LINE__, t0, t1,t2,t3);
	
	return 0;
}
/* added end by liyibo for doze mode at 2014-06-25 */
static int __devinit synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&resume_work, synaptics_rmi4_resume_work);
#endif	
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	mutex_unlock(&exp_data.mutex);
       finger_proc_write(); /* added begin by liyibo for doze mode at 2014-06-25 */
	rmi4_data->touch_stopped = false;
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && rmi4_data) 
	{
		...
		{
			pr_info("mdss %s : synaptics_rmi4_resume \n", __func__);
#ifdef CONFIG_PROJECT_P892S10_ZTE_TEST
			queue_work(resume_workqueue, &resume_work);
#else
			synaptics_rmi4_resume(&(rmi4_data->input_dev->dev));
#endif
		...
		{
			pr_info("mdss %s : synaptics_rmi4_suspend\n", __func__);
			synaptics_rmi4_suspend(&(rmi4_data->input_dev->dev));
#ifdef CONFIG_PROJECT_P892S10_ZTE_TEST
			synaptics_suspend_flag = 1;
#endif
		}
	}
