...
/* ZTE_MODIFY end */
	...
	.insert_detect = true,
	.swap_gnd_mic = NULL,
	/* ZTE_Modify for headset false detect by lvrongguo 20140317 begin */
	#if 1
	...
	.cs_enable_flags = 0,
	#endif
	/* ZTE_Modify for headset false detect by lvrongguo 20140317 end */
...
static int msm8226_configure_tert_mi2s_gpio(void)
{
	...
	int i;
	for(i = 0; i < ARRAY_SIZE(tert_mi2s_gpio);i++){
		rtn = gpio_request(tert_mi2s_gpio[i].gpio_no,tert_mi2s_gpio[i].gpio_name);
		pr_info("%s : gpio = %d,gpio_name %s, rtn = %d\n",__func__,tert_mi2s_gpio[i].gpio_no,
			tert_mi2s_gpio[i].gpio_name,rtn);
		if(rtn){
...
static struct snd_soc_dai_link msm8226_common_dai[] = {
	...
	/* Hostless I2S purpose */
	{
		.name = "Tertiary MI2S Hostless",
		.stream_name = "Tertiary MI2S Hostless",
