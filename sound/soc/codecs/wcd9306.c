...
#include "audience/es325-export.h"
#endif
...
#elif defined(CONFIG_SND_SOC_ES325) && defined(CONFIG_SND_SOC_ES_SLIM)

#define CONFIG_SND_SOC_ESXXX
#define REMOTE_ROUTE_ENABLE_CB		escore_remote_route_enable
...
#define REMOTE_ADD_CODEC_CONTROLS_CB	es325_remote_add_codec_controls
...
/* ZTE_modif add for TPA9890 state control begin */
#ifdef CONFIG_SND_SOC_TFA9890
static unsigned TPA9890_STATE = 0;
static int tapan_get_tpa9890_state(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = TPA9890_STATE;    
	return 0;
}

static int tapan_put_tpa9890_state(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	TPA9890_STATE = ucontrol->value.integer.value[0];    
	return 0;
}
#endif
/* ZTE_modif add for TPA9890 state control end */
...
static const struct snd_kcontrol_new tapan_9306_snd_controls[] = {
	...
	SOC_SINGLE_EXT("COMP2 Switch", SND_SOC_NOPM, COMPANDER_2, 1, 0,
        ...
        /* ZTE_modif add for TPA9890 state control begin */
        #ifdef CONFIG_SND_SOC_TFA9890
        SOC_SINGLE_EXT("TFA9890 state", SND_SOC_NOPM, 0, 3, 0, tapan_get_tpa9890_state,
		tapan_put_tpa9890_state),
        #endif
        /* ZTE_modif add for TPA9890 state control end */
};
...
#ifdef CONFIG_SND_SOC_ESXXX
		if ((w->shift == AIF1_PB)||(w->shift == AIF2_PB))
	...
	const char *name)
{
	...
	for (i = 0; i < core->num_of_supplies; i++) {
                printk("%s, supply_name=%s", __func__, core->supplies[i].supply);
