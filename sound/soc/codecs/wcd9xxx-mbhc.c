				   ...
				   bool norel)
{
	...
	override_bypass = (snd_soc_read(mbhc->codec,WCD9XXX_A_CDC_MBHC_B1_CTL) & 0x04) ? true : false;
...
static void wcd9xxx_swch_irq_handler(struct wcd9xxx_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == PLUG_TYPE_NONE) && insert) {
		/* ZTE_Modify for flutter by qualcom 20140529 end */
		pr_debug("%s: ************ delay 300ms **********\n", __func__);
		msleep(300);
		/* ZTE_Modify for flutter by qualcom 20140529 end */
				    ...
				    uint32_t *zr)
{
	...
	bool override_en;   /* ZTE_Modify for headset false detect by lvrongguo 20140317 */
	...
	wcd9xxx_onoff_ext_mclk(mbhc, true);
       ...
       /* ZTE_Modify for headset false detect by lvrongguo 20140317 begin */
	override_en = (snd_soc_read(codec, WCD9XXX_A_CDC_MBHC_B1_CTL) & 0x04) ? true : false;
