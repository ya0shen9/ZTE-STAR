...
int escore_i2c_boot_finish(struct escore_priv *escore)
{
	...
	do {
		pr_debug("%s(): write ES_SYNC_CMD = 0x%08x\n",
				__func__, sync_cmd);
		rc = escore_i2c_cmd(escore, sync_cmd, 0, &sync_ack);
		if (rc < 0) {
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (escore_priv.pri_intf == ES_I2C_INTF) {
		escore_priv.bus.ops.read = escore_i2c_read;
		escore_priv.bus.ops.write = escore_i2c_write;
		escore_priv.bus.ops.cmd = escore_i2c_cmd;
		escore_priv.dev = &i2c->dev;
		escore_priv.streamdev = es_i2c_streamdev;
	}

	if (escore_priv.high_bw_intf == ES_I2C_INTF) {
		escore_priv.bus.ops.high_bw_write = escore_i2c_write;
		escore_priv.boot_ops.setup = escore_i2c_boot_setup;
		escore_priv.boot_ops.finish = escore_i2c_boot_finish;
