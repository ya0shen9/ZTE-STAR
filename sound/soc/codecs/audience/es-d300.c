...
static int es300_codec_stop_algo(struct escore_priv *escore)
{
	...
	ret = escore_cmd(escore, cmd);
	mutex_unlock(&escore->api_mutex);
	if (ret)
		pr_info("%s: algo stop failed\n", __func__);
	return ret;
}
...
static int convert_output_mux_to_cmd(struct escore_priv *escore, int reg)
{
	...
	case VP:
		break;
	case AudioZoom:
		if (path_id == ES300_FEOUT1) {
			ch_mgr = TXCHMGR1;
			msg[1] &= ES300_PATH_ID(0, 0);
			msg[1] |= ES300_PATH_ID(ch_mgr, path_id);
		}
		break;
	case MM:
...
static int es_clear_route(struct escore_priv *escore)
{
	...
	return escore_flush_msg_list(escore);
}
...
void es_d300_fill_cmdcache(struct snd_soc_codec *codec)
{
	unsigned int reg;
	/*unsigned int value;*/

	for (reg = ES_CHANGE_STATUS ; reg < ES_API_ADDR_MAX ; reg++) {
