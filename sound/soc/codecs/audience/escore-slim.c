...
int escore_slim_boot_setup(struct escore_priv *escore)
{
	...
	rc = escore_slim_cmd(escore, boot_cmd, 0, &boot_ack);
	if (rc < 0) {
...
int escore_slim_boot_finish(struct escore_priv *escore)
{
	...
	do {
		pr_debug("%s(): write ES_SYNC_CMD = 0x%08x\n",
				__func__, sync_cmd);
		rc = escore_slim_cmd(escore, sync_cmd, 0, &sync_ack);
		if (rc < 0) {
...
static int escore_slim_dev_up_thread(void *priv)
{
...
static int register_snd_soc(struct escore_priv *escore_priv)
{
	...
	rc = snd_soc_register_codec(&sbdev->dev,
			...
			escore_priv->dai, escore_priv->dai_nr);
...
static int escore_slim_probe(struct slim_device *sbdev)
{
	...
	init_completion(&escore_priv.probe_compl);
	if (escore_priv.pri_intf == ES_SLIM_INTF) {
		escore_priv.bus.ops.read = escore_slim_read;
		escore_priv.bus.ops.write = escore_slim_write;
		escore_priv.bus.ops.cmd = escore_slim_cmd;
		escore_priv.dev = &escore_priv.gen0_client->dev;
	}
	if (escore_priv.high_bw_intf == ES_SLIM_INTF) {
		escore_priv.bus.ops.high_bw_write = escore_slim_write;
		escore_priv.boot_ops.setup = escore_slim_boot_setup;
		escore_priv.boot_ops.finish = escore_slim_boot_finish;
	...
	thread = kthread_run(escore_slim_dev_up_thread, &escore_priv,
			"audience thread");
	if (IS_ERR(thread)) {
		pr_err("%s(): can't create firmware thread:%p\n",
				__func__, thread);
		return -ENOMEM;
	}
#endif
...
static int escore_slim_device_up(struct slim_device *sbdev)
{
	...
	if (!priv->slim_setup)
		wait_for_completion(&priv->probe_compl);

	priv->slim_setup(priv);
	if (escore_priv.high_bw_intf == ES_SLIM_INTF) {
		rc = fw_download(priv);
		BUG_ON(rc != 0);
	}
