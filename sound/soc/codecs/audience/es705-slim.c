...
static int es705_dt_get_platform_data(struct device *dev,
		struct esxxx_platform_data *pdata);
static int es705_dt_get_slim_interface_dev_info(struct device *dev,
			     ...
			     unsigned int ch_cnt, unsigned int rate)
{
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		ch_h[i] = rx[idx].ch_h;
		sph[i] = rx[idx].sph;
		dev_info(&sbdev->dev, "%s(): ch_h = %d, sph = %x\n",
			__func__, ch_h[i], sph[i]);
	}
...
static int es705_slim_read(struct es705_priv *es705, void *rspn, int len)
{
	...
	for (i = 0; i < MAX_SMB_TRIALS; i++) {
		...
		rc = slim_get_logical_addr(sbdev, es705->gen0_client->e_addr,
	...
	if (!rc){
		...
		if (es705->slim_log){
			for (iCount = 0; iCount < len; iCount++){
				dev_info(&sbdev->dev,
				"%s(): SLIM read success: value[%d] =0x%02x\n",
				__func__, iCount, ((u8*)rspn)[iCount]);
			}
			#endif
		}
	return rc;
				...
				const void *buf, int len)
{
	...
	BUG_ON(len < 0);
	 ...
	 if (es705->slim_log){
		...
		for (iCount = 0; iCount < len; iCount++){
			dev_info(&sbdev->dev,
				"%s(): SLIM write: value[%d]=%02x\n",
					__func__, iCount, ((u8*)buf)[iCount]);
		}
	 }
#endif
	...
	while (wr < len) {
		...
		for (i = 0; i < MAX_SMB_TRIALS; i++) {
			...
			slim_get_logical_addr(sbdev, sbdev->e_addr,
					6, &(sbdev->laddr));
	...
	if(1){	 
		dev_info(&sbdev->dev,
		"%s(): SLIM write FAIL: value[%d] = %02x,  retval=%d\n",
		__func__, wr, ((u8*)buf)[wr], rc);
	}
	#endif
					...
					u32 *rspn, int match)
{
	...
	if (rc){
		dev_err(&sbdev->dev, "%s(): SlimWrite Failed: retval=%d\n", __func__,rc);
		for (iCount=0; iCount < len; iCount++)
		{
			dev_info(&sbdev->dev,
				"%s(): SLIM write command: value[%d]=%02x\n",
					__func__, iCount, ((u8*)buf)[iCount]);
		}
		return rc;
		}
	...
	if (trials == MAX_WRITE_THEN_READ_TRIALS){
		dev_err(&sbdev->dev, "%s(): SLIMRead Failed: retval=%d  max trials %d\n", __func__, rc, trials);
		for (iCount=0; iCount < len; iCount++)
			dev_info(&sbdev->dev,
				"%s(): SLIMRead failed for write command: value[%d]=%02x\n",
					__func__, iCount, ((u8*)buf)[iCount]);
	}
	return rc;
}
...
static int es705_slim_boot_setup(struct es705_priv *es705)
{
	...
	dev_info(es705->dev, "%s(): SYNC_SBL success rc=%d  retry= %d\n",
								__func__,rc, retry);
#endif
	es705->mode = SBL;
	...
	if (rc){
		retry=3;
		ack_rspn=ES705_BOOT_ACK;
		do {
		response=0;
		rc = es705_slim_read(es705, &response, sizeof(response));
		dev_info(es705->dev, "%s(): inner rc=%d  response= %x\n",
								__func__,rc, response);
		if ((!rc)&&(response==ack_rspn))
			break;
		}while(--retry);
	}
	dev_dbg(es705->dev, "%s(): BOOT_CMD try= %d\n",
							__func__, retry);
#endif
...
#define SUPPORT_ES704_ES705
static int es705_fw_thread(void *priv)
{
	struct es705_priv *es705 = (struct es705_priv  *)priv;
	int retrynum = 200;
	int i;
#ifdef SUPPORT_ES704_ES705
	u8* pt_id_change=&(es705->gen0_client->e_addr[2]);
	dev_info(&es705->gen0_client->dev, "%s(), gerneric id need be modified = 0x%02x\n", __func__, *pt_id_change);
#endif
	i = 0;
	do {
		slim_get_logical_addr(es705->gen0_client,
		                                      ...
		                                      6, &(es705->gen0_client->laddr));
#ifdef SUPPORT_ES704_ES705
		if (es705->gen0_client->laddr == 0xf0) {
			usleep_range(1000, 2000);
			*pt_id_change ^= 0x80;
			dev_info(&es705->gen0_client->dev, "%s(), generic id after modified = 0x%02x\n", __func__, *pt_id_change);
		}
#else
		usleep_range(1000, 2000);
#endif
	} while ((es705->gen0_client->laddr == 0xf0)&&(i++ < retrynum));
	dev_dbg(&es705->gen0_client->dev, "%s(): gen0_client LA = %d\n",
			__func__, es705->gen0_client->laddr);
	if (es705->gen0_client->laddr == 0xf0)
		goto error_polling_LA;

#ifdef SUPPORT_ES704_ES705
	pt_id_change=&(es705->intf_client->e_addr[2]);
	dev_info(&es705->intf_client->dev, "%s(), interface id need be modified = 0x%02x\n", __func__, *pt_id_change);
#endif
	i = 0;
	do {
...
#ifdef SUPPORT_ES704_ES705
		if (es705->intf_client->laddr == 0xf0) {
			usleep_range(1000, 2000);
			*pt_id_change ^= 0x80;
			dev_info(&es705->intf_client->dev, "%s(), interface id after modified = 0x%02x\n", __func__, *pt_id_change);
		}
#else
		usleep_range(1000, 2000);
#endif
	} while ((es705->intf_client->laddr == 0xf0)&&(i++ < retrynum));
						...
						struct slim_device *slim_ifd)
{
	...
	/*ZTE_modify by wangtao for slimbus addr of es704 or es705*/
	if((hw_ver & 0x0003) >0)
		prop = of_find_property(dev->of_node, "elemental-addr", NULL);
	else
		prop = of_find_property(dev->of_node, "elemental-705-addr", NULL);
...
extern int es705_mclk_enable( int enable);
static int es705_dt_get_platform_data(struct device *dev,
		struct esxxx_platform_data *pdata)
{
	const void *ptr;
	...
	ptr = of_get_property(dev->of_node, "ext_clk_rate", NULL);
	if (ptr)
		pdata->ext_clk_rate = (u8)be32_to_cpup(ptr);
	else
		dev_err(dev, "%s():Unable to read ext clk rate\n", __func__);

	pdata->irq_base = gpio_to_irq(pdata->gpiob_gpio);
	pdata->esxxx_clk_cb = es705_mclk_enable;

	dev_info(dev, "%s(): reset_gpio: %d pdata->wakeup_gpio: %d\n", __func__,
		pdata->reset_gpio, pdata->wakeup_gpio);
	dev_info(dev, "%s(): gpioa_gpio: %d pdata->gpiob_gpio: %d\n", __func__,
		pdata->gpioa_gpio, pdata->gpiob_gpio);

	if (pdata->reset_gpio < 0 || pdata->wakeup_gpio < 0 ||
			pdata->gpioa_gpio < 0 || pdata->gpiob_gpio < 0) {
		dev_err(dev, "%s(): reset_gpio: %d pdata->wakeup_gpio: %d\n",
			__func__, pdata->reset_gpio, pdata->wakeup_gpio);
		dev_err(dev, "%s(): gpioa_gpio: %d pdata->gpiob_gpio: %d\n",
			__func__, pdata->gpioa_gpio, pdata->gpiob_gpio);
		return -EIO;
	}
...
static int es705_slim_probe(struct slim_device *sbdev)
{
	...
	if (sbdev->dev.of_node) {
		dev_dbg(&sbdev->dev, "%s(): Platform Data from DeviceTree",
					__func__);

		pdata = devm_kzalloc(&sbdev->dev, sizeof(*pdata), GFP_KERNEL);
		if (!pdata) {
			dev_err(&sbdev->dev,
			"%s(): platform data allocation failed\n", __func__);
			rc = -ENOMEM;
			goto pdata_error;
		}

		/* Platform data is present only within gen0 node in dts file */
		if (strncmp(sbdev->name, "es705-codec-gen0",
				SLIMBUS_NAME_SIZE) == 0) {
			rc = es705_dt_get_platform_data(&sbdev->dev, pdata);
			if (rc) {
		...
		rc = es705_dt_get_slim_interface_dev_info(&sbdev->dev, sbdev);
		if (rc) {
			dev_err(&sbdev->dev,
			"%s(): Error, parsing slim interface\n", __func__);
			devm_kfree(&sbdev->dev, pdata);
			goto pdata_error;
		}
		...
		if (strncmp(sbdev->name, "es705-codec-intf",
				SLIMBUS_NAME_SIZE) == 0) {
...
int es705_bus_init(struct es705_priv *es705)
{
	...
	if (!rc) {
		dev_dbg(es705->dev, "%s(): registered as SLIMBus", __func__);
