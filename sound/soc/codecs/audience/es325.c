...
int es325_uart_check_sbl_mode(struct escore_priv *es325)
{
	...
	rc = es325->bus.ops.write(es325, &msg, 1);
	if (rc < sizeof(msg)) {
	...
	/* Read SBL sync ack */
	rc = es325->bus.ops.read(es325, &msg, 1);
	if (rc < 0) {
...
int es325_uart_stage1_fw_download(struct escore_priv *es325)
{
	...
	escore_configure_tty(es325->uart_dev.tty, UART_TTY_BAUD_RATE_3_M,
				UART_TTY_STOP_BITS);
...
int es325_bootup(struct escore_priv *es325)
{
	...
	rc = es325->bus.ops.write(es325, (char *)es325->standard->data,
						es325->standard->size);
	if (rc < 0) {
...
int es325_boot_finish(struct escore_priv *es325)
{
	...
	if (es325->uart_boot_finish)
		es325->uart_boot_finish(es325);
	/* sometimes i2c sends success in second sync command */
	do {
		pr_info("%s(): write ES_SYNC_CMD = 0x%08x\n",
			__func__, sync_cmd);
		rc = escore_cmd(es325, sync_cmd);
		if (rc < 0) {
			pr_err("%s(): firmware load failed sync write\n",
				__func__);
			continue;
		}
		sync_ack = es325->bus.last_response;
...
static int es325_sleep(struct escore_priv *es325)
{
	...
	rc = escore_cmd(es325, smooth_mute);
	if (rc < 0) {
...
int es325_set_streaming(struct escore_priv *es325, int value)
{
	return escore_cmd(es325,
		es325_streaming_cmds[es325->streamdev.intf] | value);
}
...
int es325_core_probe(struct device *dev)
{
	...
	escore_priv.boot_ops.finish = es325_boot_finish;
	...
	escore_priv.uart_dev.baudrate_bootloader = UART_TTY_BAUD_RATE_28_8_K;
