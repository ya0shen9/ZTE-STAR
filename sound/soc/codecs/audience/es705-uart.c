...
synca_retry:
	...
	if (rc < 1) {
		iRetry++;
		dev_err(es705->dev, "%s(): sync ack rc = %d  retry=%d\n",
				__func__, rc, iRetry);
		if (iRetry < 10)
			goto synca_retry;
		goto es705_bootup_failed;
...
bootb_retry:
	...
	if (rc < 1) {
		...
		dev_err(es705->dev, "%s(): sbl boot nack rc %d   retry=%d\n",
				__func__, rc, iRetry);
		if (iRetry < 10)
			goto bootb_retry;
		goto es705_bootup_failed;
...
int es705_uart_boot_finish(struct es705_priv *es705)
{
	...
	if (es705->uart_app_rate != UART_RATE_3kk) {
		es705_configure_tty(es705_priv.uart_dev.tty,
...
sync_again:
	...
	if (rc < 0) {
		dev_err(es705->dev,
		"%s(): firmware load failed (no sync response) retry=%d\n",
			__func__, iretry1 + 1);
		if (iretry1++ < iretrytimes)
			goto sync_again;
		goto es705_bootup_failed;
	}
	if (sync_cmd != sync_resp) {
		dev_err(es705->dev,
		"%s(): firmware load failed (invalid sync response) retry=%d\n",
			__func__, iretry2 + 1);
		dev_err(es705->dev, "%s(): firmware load failed (response was: 0x%08x)\n",
			__func__, sync_resp);
		if (iretry2++ < iretrytimes)
			goto sync_again;
		goto es705_bootup_failed;
	}

	dev_dbg(es705->dev, "%s(): ******firmware load success******\n", __func__);
...
int es705_uart_wakeup(struct es705_priv *es705)
{
	int rc;
	char wakeup_char = 0x81;//gouyajun0406

	unsigned int temp;

	temp = es705_priv.uart_app_rate;//gouyajun0406
	es705_priv.uart_app_rate = UART_RATE_4608;//gouyajun0406

	rc = es705_uart_open(es705);
	if (rc) {
		dev_err(es705->dev, "%s(): uart open error\n",
			__func__);
		goto es705_uart_wakeup_exit;
	}
...
es705_uart_wakeup_exit:
	rc = es705_uart_close(es705);
	if (rc)
		dev_err(es705->dev, "%s(): uart close error\n",
			__func__);
	es705_priv.uart_app_rate = temp;//gouyajun0406
	return rc;
}
...
bootup_error:
	/* close filp */
	filp_close(es705_priv.uart_dev.file, 0);
cdev_init_error:
...
static int es705_uart_remove(struct platform_device *dev)
{
	...
	if (es705_priv.uart_dev.ld)
		tty_ldisc_deref(es705_priv.uart_dev.ld);

	if (es705_priv.uart_dev.file)
		rc = filp_close(es705_priv.uart_dev.file, 0);
...
static struct esxxx_platform_data esxxx_platform_data = {
	...
	.gpioa_gpio     = -1,
	.gpiob_gpio     = -1,
	.esxxx_clk_cb   = es705_mclk_enable,
};
