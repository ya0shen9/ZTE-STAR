...
static int streaming_open(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&es705->overall_mutex);//Richard 4.23
	/*
	...
	if (!es705->streamdev.read  ||
		...
		mutex_unlock(&es705->overall_mutex);//Richard 4.23
		return -ENODEV;
	}
					...
					size_t count, loff_t *f_pos)
{
	...
	BUG_ON(!es705);
	if ((0==es705->non_vs_data_access) && (es705->es705_power_state !=
					     ES705_SET_POWER_STATE_VS_OVERLAY)){/*Richard 3.29*/
			pr_err("%s(): not in VS mode, can't read data block\n",
			__func__);
				...
				size_t count, loff_t *f_pos)
{
	...
	if ((0==es705->non_vs_data_access) && (es705->es705_power_state !=
					     ES705_SET_POWER_STATE_VS_OVERLAY)){/*Richard 3.29*/
			pr_err("%s(): not in VS mode, can't write data block\n",
			__func__);
			return -EPERM;/*Richard 0327*/
	}	
...
static int datablock_opened=0;
#ifdef USING_1M_STREAMING//Richard 4.23
static unsigned int saved_rate;
#endif
static int datablock_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("%s(): try to open data block\n",
			__func__);
	mutex_lock(&es705->overall_mutex);

	if ((0==es705->non_vs_data_access) && (es705->es705_power_state !=
					     ES705_SET_POWER_STATE_VS_OVERLAY)){/*Richard 3.29*/
			pr_err("%s(): not in VS mode, can't open data block\n",
			__func__);
...
#ifdef USING_1M_STREAMING//Richard 4.23
		saved_rate = es705->uart_app_rate;
                if(es705_priv.es705_power_state ==
				ES705_SET_POWER_STATE_VS_OVERLAY)
			es705->uart_app_rate = UART_RATE_1kk;//kylin
		else
                        es705->uart_app_rate = UART_RATE_3kk;//kylin
#endif
...
static int datablock_release(struct inode *inode, struct file *filp)
{
	...
	BUG_ON(!es705);
	if (!datablock_opened){
			pr_err("%s(): datablock not opened, can't release data block\n",
			__func__);
			mutex_unlock(&es705->overall_mutex);
			return -EPERM;/*Richard 0327*/
	}
	datablock_opened = 0;
#ifdef USING_1M_STREAMING//Richard 4.23
	es705_priv.uart_app_rate = saved_rate;//kylin20140422
#endif
	mutex_unlock(&es705->api_mutex);
	if (es705->datablockdev.close)
		es705->datablockdev.close(es705);
	dev_info(es705->dev, "%s(): release complete\n", __func__);
	mutex_unlock(&es705->overall_mutex);
	return 0;
}
